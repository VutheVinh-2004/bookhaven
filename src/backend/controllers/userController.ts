...
export const getAllUsers = (req: AuthRequest, res: Response) => {
  ...
  return ok(res, db.prepare(`${query} ORDER BY created_at DESC`).all());
};
...
export const updateUserRole = (req: AuthRequest, res: Response) => {
  ...
  if (!["user", "admin"].includes(role)) return fail(res, 400, "Vai trò không hợp lệ.");
  if (id === req.user?.id && role !== "admin") {
    return fail(res, 400, "Không thể tự hạ quyền tài khoản của chính mình.");
  }
...
export const deleteUser = (req: AuthRequest, res: Response) => {
  const id = Number(req.params.id);
  if (!isPositiveInt(id)) return fail(res, 400, "ID người dùng không hợp lệ.");
  if (id === req.user?.id) return fail(res, 400, "Không thể vô hiệu hóa tài khoản của chính mình.");
